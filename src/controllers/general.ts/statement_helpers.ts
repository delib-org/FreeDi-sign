...
export function newSection({
	...
	parentDocumentId,
	order,
}: NewSection): Statement | undefined {
	try {
...
export function newParagraph({
	...
	parentDocumentId,
	order,
}: NewParagraphProps): Statement | undefined {
	try {
		const creator = store.getState().user.user;
		if (!creator) throw new Error('User not found');
		const creatorId = creator.uid;
...
export function statementsToDocument({
	section,
	statements,
	level = 1,
}: StatementsToDocumentProps): DocumentObject | undefined {
	try {
		...
		const levelStatements = statements.filter(
			(st) =>
				st.statementType === StatementType.document &&
				st.parentId === section.statementId
		) as Statement[];

		const sections = levelStatements
			.filter((st) => st.documentSettings?.type === DocumentType.section)
			.sort(
				(a, b) =>
					(a.documentSettings?.order || 0) - (b.documentSettings?.order || 0)
			) as Statement[];

		const paragraphs = levelStatements
			.filter((st) => st.documentSettings?.type === DocumentType.paragraph)
			.sort(
				(a, b) =>
					(a.documentSettings?.order || 0) - (b.documentSettings?.order || 0)
			) as Statement[];
		...
		const document: DocumentObject = {
			...
			sections: sections
				.map((section) =>
					statementsToDocument({ section, statements, level: level + 1 })
				)
				.filter((d) => d !== undefined) as DocumentObject[],
		};
		return document;
	} catch (error) {
		console.error(error);
...
interface CreateNewStatementProps {
	title: string;
	description?: string;
	statement: Statement;
	order: number;
...
export function createNewStatement({
	...
	user,
	userData,
}: CreateNewStatementProps): Statement | undefined {
	try {
		...
		const parentDocumentId =
			statement.documentSettings?.parentDocumentId || statement.statementId;
