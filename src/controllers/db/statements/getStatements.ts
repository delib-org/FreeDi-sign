...
export function listenToUserTopStatements(
	unsubscribes: UnsubscribeObject[],
		...
		return onSnapshot(q, (subscriptionsDB: QuerySnapshot<DocumentData>) => {
			subscriptionsDB.docChanges().forEach((change) => {
				...
				if (change.type === 'added') {
					const unsubFunction: Unsubscribe = listenToStatement(
						statementSubscription.statementId
					);

					const index = unsubscribes.findIndex(
...
export function listenToDocument(statementId: string | undefined): Unsubscribe {
	try {
		...
		const q = query(
			...
			where('statementSettings.show', '==', true),
			orderBy('documentSettings.order', 'asc')
		);
		return onSnapshot(
			q,
			(documentsDB) => {
